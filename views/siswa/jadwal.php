...
if ($siswa) {
    ...
    $ks->execute([$siswa['id']]);
    $ksRow = $ks->fetch(PDO::FETCH_ASSOC);
    if ($ksRow) {
        $namaKelas = $ksRow['nama_kelas'];
        $st = $db->prepare("
            SELECT
                j.*,
                m.nama_mapel,
...
<div class="dash-header-row">
    ...
    <p style="color:var(--gray-500);font-size:13px;">Kelas <?= htmlspecialchars($namaKelas ?: '-') ?></p>
    </div>
...
<div class="card">
    ...
    <div class="card-body">
        <?php if (empty($jadwal)): ?>
            <p class="text-muted" style="padding:10px;background:var(--gray-50);border-radius:var(--radius-sm);">Belum ada jadwal untuk kelas ini.</p>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($jadwal as $i => $j): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($j['kode_mapel'] ?? ($j['kode_mapel']??'-')) ?></td>
...
require VIEW_PATH . '/layouts/mobile.php';
?>
